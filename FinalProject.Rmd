...
```{r}
#v<-list(44.009033701591235,-73.18251784589387,44.01077630208863,-73.1810714919134,44.01344766959134,-73.18128143605556,44.015309452339565,-73.16813214614554,44.00160945176687,-73.17739168604255)

#m<-matrix(v, ncol = 2,)
#data.frame(m, row.names = Places, col.names)
...
Midd <- leaflet(dfMidd)%>%
  ...
  addCircleMarkers(
    ...
    label = "Ridgeline",
    labelOptions = labelOptions(noHide = T)) %>%
  #addPopups(-73.18251784589387, 44.009033701591235, content,
    #options = popupOptions(closeButton = T))%>%
  addCircleMarkers(
    lng = -73.1810714919134, lat = 44.01077630208863,
    label = "Ross",
    labelOptions = labelOptions(noHide = T, textOnly = TRUE)) %>%
  #addPopups(-73.1810714919134, 44.01077630208863, content,
    #options = popupOptions(closeButton = T))%>%
  addCircleMarkers(
